...
Expr List::parse(Assoc &env)
{
    ...
    else
    {
            ...
            else
            {
                ...
                case E_COND:
                {
                    ...
                    for (size_t i = 1; i < stxs.size(); ++i)
                    {
                        ...
                        for (auto &cstx : cl->stxs)
                            clause_exprs.push_back(cstx->parse(env));
                ...
                case E_LAMBDA:
                {
                    ...
                    {
                        new_env = extend(param, VoidV(), new_env);
                    }

                    if (stxs.size() == 3)
                    {
                        Expr body = stxs[2]->parse(new_env); // 使用扩展后的环境
                    ...
                    else
                    {
                        ...
                        {
                            bodies.push_back(stxs[i]->parse(new_env)); // 使用扩展后的环境
                        }
                ...
                {
                    if (stxs.size() < 3)
                        throw RuntimeError("define requires a name and a value");
                ...
                {
                    if (stxs.size() != 3)
                        throw RuntimeError("set! requires a name and a value");
                    SymbolSyntax *name = dynamic_cast<SymbolSyntax *>(stxs[1].get());
                    if (!name)
                        throw RuntimeError("set! first argument must be a symbol");
        ...
        if (primitives.count(op) != 0)
        {
            ...
            {
                if (parameters.size() != 2)
                    throw RuntimeError("Wrong number of arguments for setcar");
                return Expr(new SetCar(parameters[0], parameters[1]));
            }
            else if (op_type == E_SETCDR)
            {
                if (parameters.size() != 2)
                    throw RuntimeError("Wrong number of arguments for setcdr");
